...
from hawkes_tools.base import BaseEstimator, normalize_events
...
class ModelHawkesSumExpKernLeastSq(ModelHawkes):
    ...
    def cast_period_length(self, period_length: float | None = None) -> float:
        """Return the internal sentinel for an unset varying-baseline period."""
...
def _sumexp_least_squares_one_quad(realization, end_time, baseline, adjacency, decays, period_length):
    value, _, _ = _sumexp_least_squares_varying_baseline_segmented(
        realization, end_time, baseline, adjacency, decays, period_length
    )
    return value
...
def _sumexp_least_squares_grad_one_quad(realization, end_time, baseline, adjacency, decays, period_length):
    _, grad_baseline, grad_adjacency = _sumexp_least_squares_varying_baseline_segmented(
        realization, end_time, baseline, adjacency, decays, period_length
    )
    return grad_baseline, grad_adjacency


def _sumexp_least_squares_varying_baseline_segmented(
    realization,
    end_time,
    baseline,
    adjacency,
    ...
    if baseline_arr.ndim != 2:
        raise ValueError("varying baseline must be a 2-dimensional array")
    if period_length is None:
        raise ValueError("period_length is required for varying baselines")
    n_nodes, n_baselines = baseline_arr.shape
    n_decays = np.asarray(decays, dtype=float).size
    decays_arr = np.asarray(decays, dtype=float)
    decays_flat = np.tile(decays_arr, n_nodes)
    event_arrays = [np.asarray(ts, dtype=float) for ts in realization]
    segment_points = _segment_points(event_arrays, float(end_time), float(period_length), n_baselines)

    state = np.zeros((n_nodes, n_decays), dtype=float)
    event_indices = np.zeros(n_nodes, dtype=int)
    grad_baseline = np.zeros_like(baseline_arr)
    grad_adjacency = np.zeros_like(adjacency)
    value = 0.0

    decay_pair = decays_flat[:, None] + decays_flat[None, :]
    ...
    for left, right in zip(segment_points[:-1], segment_points[1:]):
        ...
        for node, timestamps in enumerate(event_arrays):
            idx = int(event_indices[node])
            while idx < timestamps.size and float(timestamps[idx]) <= left:
                state[node, :] += decays_arr
                idx += 1
            event_indices[node] = idx

        state_flat = state.ravel()
        decay_integrals = _exp_decay_integrals(decays_flat, length)
        feature_integrals = state_flat * decay_integrals
        pair_integrals = np.outer(state_flat, state_flat) * _exp_decay_integrals(decay_pair, length)
        baseline_interval = _baseline_interval_index(left + 0.5 * length, period_length, n_baselines)

        for target in range(n_nodes):
            coefficients = adjacency[target].reshape(n_nodes * n_decays)
            mu = float(baseline_arr[target, baseline_interval])
            intensity_integral = mu * length + float(np.dot(coefficients, feature_integrals))
            square_integral = (
                mu * mu * length
                + 2.0 * mu * float(np.dot(coefficients, feature_integrals))
                + float(coefficients @ pair_integrals @ coefficients)
            )
            value += square_integral
            grad_baseline[target, baseline_interval] += 2.0 * intensity_integral
            grad_adjacency[target, :, :] += (
                2.0 * (mu * feature_integrals + pair_integrals @ coefficients)
            ).reshape(n_nodes, n_decays)

        state *= np.exp(-decays_arr.reshape(1, -1) * length)

    for i in range(n_nodes):
        event_feature_sums = _sumexp_ls_event_feature_sums(event_arrays, decays_arr, i)
        value -= 2.0 * sum(
            _sumexp_intensity_from_coeffs(
                event_arrays, baseline_arr, adjacency, decays_arr, i, float(t), period_length
            )
            for t in event_arrays[i]
        )
        for b in range(n_baselines):
            ...
            grad_baseline[i, b] -= 2.0 * event_sum
        grad_adjacency[i, :, :] -= 2.0 * event_feature_sums
    return float(value), grad_baseline, grad_adjacency


def _exp_decay_integrals(decay, length):
    decay = np.asarray(decay, dtype=float)
    length = float(length)
    return np.where(
        decay > 0.0,
        (1.0 - np.exp(-decay * length)) / decay,
        length,
    )
...
def _segment_points(realization, end_time, period_length, n_intervals):
    event_times = np.concatenate(
        [ts for ts in realization if ts.size] or [np.asarray([], dtype=float)]
    )
    points = [0.0, float(end_time)]
    points.extend(float(t) for t in event_times if 0.0 < float(t) < end_time)
    if period_length is not None and n_intervals > 1:
        step = float(period_length) / int(n_intervals)
        if step > 0.0:
            k = 1
