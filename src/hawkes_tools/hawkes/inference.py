...
from hawkes_tools.base import BaseEstimator, History, normalize_events, relative_distance
...
class _LearnerBase(BaseEstimator):
    ...
    def plot_estimated_intensity(self, events=None, intensity_track_step=None, end_time=None, **kwargs):
        from hawkes_tools.plot import plot_estimated_intensity
    ...
    def qq_plots(self, events=None, end_times=None, **kwargs):
        from hawkes_tools.plot import qq_plots
...
def _hawkes_em_step_reference(events, sizes, end_times, baseline, kernel, discretization):
    ...
    for r in range(n_realizations):
        for u in range(n_nodes):
            last_indices = np.asarray(sizes[r], dtype=int) - 1
            for event_index in range(int(sizes[r, u]) - 1, -1, -1):
                t = float(events[r, u, event_index])
                contributions = np.zeros((n_nodes, kernel_size), dtype=float)
                intensity = 0.0
                for v in range(n_nodes):
                    source_index = int(last_indices[v])
                    while source_index >= 0 and float(events[r, v, source_index]) > t:
                        source_index -= 1
                    last_indices[v] = source_index
                    while source_index >= 0:
                        if u == v and event_index == source_index:
...
def _hawkes_em_step_numba_impl(events, sizes, end_times, baseline, kernel, discretization, out_baseline, out_kernel):
    ...
    is_zero_based_uniform = discretization[0] == 0.0
    dt0 = discretization[1] - discretization[0]
    if dt0 <= 0.0:
        is_zero_based_uniform = False
    if is_zero_based_uniform:
        for m in range(1, kernel_size):
            if abs((discretization[m + 1] - discretization[m]) - dt0) > 1e-12:
                is_zero_based_uniform = False
                break

    for r in range(n_realizations):
        for u in range(n_nodes):
            last_indices = np.empty(n_nodes, dtype=np.int64)
            contributions = np.empty((n_nodes, kernel_size), dtype=np.float64)
            for v in range(n_nodes):
                last_indices[v] = sizes[r, v] - 1
            for event_index in range(sizes[r, u] - 1, -1, -1):
                t = events[r, u, event_index]
                intensity = 0.0
                for v in range(n_nodes):
                    for m in range(kernel_size):
                        contributions[v, m] = 0.0
                for v in range(n_nodes):
                    source_index = last_indices[v]
                    while source_index >= 0 and events[r, v, source_index] > t:
                        source_index -= 1
                    last_indices[v] = source_index
                    while source_index >= 0:
                        if u == v and event_index == source_index:
                        ...
                        if lag == 0.0:
                            m = 0
                        elif is_zero_based_uniform:
                            m = int(lag / dt0)
                        else:
                            for boundary in range(1, discretization.size):
...
class HawkesConditionalLaw(_LearnerBase):
    ...
    def get_kernel_values(self, i, j, abscissa_array):
        ...
        if str(self.quad_method).lower() == "log":
            ...
            return np.power(10.0, log_interpolation)
...
class HawkesCumulantMatching(_LearnerBase):
    ...
    _allowed_solvers = ("momentum", "adam", "adagrad", "rmsprop", "adadelta", "gd")
    _allowed_penalties = {None, "none", "l1", "l2", "elasticnet"}
    ...
    def __init__(
        ...
        tol = self._nonnegative_finite("tol", tol)
        max_iter = self._integer_at_least("max_iter", max_iter, 0)
        print_every = self._integer_at_least("print_every", print_every, 1)
        record_every = self._integer_at_least("record_every", record_every, 1)
        super().__init__(tol, max_iter, verbose, print_every, record_every)
        self.integration_support = self._positive_finite("integration_support", integration_support)
        self.C = self._positive_finite("C", C)
        self.penalty = self._normalize_penalty(penalty)
        self.solver = solver
        self.step = self._positive_finite("step", step)
        self.solver_kwargs = self._validate_solver_kwargs(solver_kwargs)
        self.cs_ratio = self._validate_cs_ratio(cs_ratio)
        self.elastic_net_ratio = self._validate_elastic_net_ratio(elastic_net_ratio)
        self.solution = None
        self._mean_intensity = None
        self._covariance = None
        self._skewness = None
        self._cumulant_computer = _HawkesCumulantComputer(self.integration_support)
    ...
    def solver(self, value):
        ...
        if solver not in self._allowed_solvers:
            raise ValueError(
                f"solver must be one of {list(self._allowed_solvers)}, received {value}"
            )
        self._solver = solver
    ...
    def _integer_at_least(name, value, minimum):
        ...
        if numeric < minimum or numeric != value:
            raise ValueError(f"{name} must be an integer >= {minimum}")
        return numeric

    @classmethod
    def _normalize_penalty(cls, value):
        if value is None:
            return None
        penalty = str(value).lower()
        if penalty not in cls._allowed_penalties:
            allowed = ["none", "l1", "l2", "elasticnet", None]
            raise ValueError(f"penalty must be one of {allowed}, received {value}")
        return penalty

    @staticmethod
    def _validate_solver_kwargs(value):
        if value is None:
            return {}
        if not isinstance(value, dict):
            raise ValueError("solver_kwargs must be a dict or None")
        return dict(value)

    @staticmethod
    def _validate_cs_ratio(value):
        if value is None:
            return None
        try:
            numeric = float(value)
        except (TypeError, ValueError) as exc:
            raise ValueError("cs_ratio must be between 0 and 1") from exc
        if not math.isfinite(numeric) or not 0.0 <= numeric <= 1.0:
            raise ValueError("cs_ratio must be between 0 and 1")
        return numeric

    @staticmethod
    def _validate_elastic_net_ratio(value):
        try:
            numeric = float(value)
        except (TypeError, ValueError) as exc:
            raise ValueError("elastic_net_ratio must be between 0 and 1") from exc
        if not math.isfinite(numeric) or not 0.0 <= numeric <= 1.0:
            raise ValueError("elastic_net_ratio must be between 0 and 1")
        return numeric
    ...
    def solve(self, adjacency_start=None, R_start=None):
        if self.data is None:
            raise ValueError("fit data before solving")
        if not self._cumulant_computer.cumulants_ready:
            self.compute_cumulants()
        start_point = self._resolve_starting_R(adjacency_start=adjacency_start, R_start=R_start)
        start_point = np.real_if_close(start_point).astype(float, copy=False)
        previous = self._objective_from_R(start_point)

        def objective_flat(flat_R):
            return self._objective_from_R(np.asarray(flat_R, dtype=float).reshape(self.n_nodes, self.n_nodes))
        ...
        self.optim_result = result
        solution = result.x.reshape(self.n_nodes, self.n_nodes)
        if not self._is_valid_solution(solution):
            solution = start_point if self._is_valid_solution(start_point) else np.eye(self.n_nodes)
        self.solution = solution
        current = self._objective_from_R(solution)
        rel_obj = abs(current - previous) / max(abs(previous), 1e-15)
        self._record(n_iter=int(getattr(result, "nit", self.max_iter)), objective=current, rel_obj=rel_obj, force=True)
        return self

    def _resolve_starting_R(self, adjacency_start=None, R_start=None):
        if adjacency_start is None and R_start is not None:
            start_point = np.asarray(R_start, dtype=float)
        elif adjacency_start is None or (isinstance(adjacency_start, str) and adjacency_start == "random"):
            start_point = self.starting_point(random=adjacency_start == "random")
        elif isinstance(adjacency_start, str):
            raise ValueError("adjacency_start must be None, 'random', or an array")
        else:
            adjacency = np.asarray(adjacency_start, dtype=float)
            if adjacency.shape != (self.n_nodes, self.n_nodes):
                raise ValueError(
                    f"adjacency_start must have shape ({self.n_nodes}, {self.n_nodes}), "
                    f"got {adjacency.shape}"
                )
            start_point = scipy.linalg.inv(np.eye(self.n_nodes) - adjacency)
        if start_point.shape != (self.n_nodes, self.n_nodes):
            raise ValueError(
                f"starting R must have shape ({self.n_nodes}, {self.n_nodes}), "
                f"got {start_point.shape}"
            )
        return start_point

    @staticmethod
    def _is_valid_solution(solution):
        if not np.all(np.isfinite(solution)):
            return False
        try:
            scipy.linalg.inv(solution)
    ...
    def objective(self, adjacency=None, R=None):
        if adjacency is not None:
            ...
            R = self.solution
        return self._objective_from_R(np.asarray(R, dtype=float))

    @property
    def strength_lasso(self):
        penalty = None if self.penalty is None else str(self.penalty).lower()
        if penalty == "elasticnet":
            return float(self.elastic_net_ratio) / float(self.C)
        if penalty == "l1":
    ...
    def strength_ridge(self):
        penalty = None if self.penalty is None else str(self.penalty).lower()
        if penalty == "elasticnet":
            return (1.0 - float(self.elastic_net_ratio)) / float(self.C)
        if penalty == "l2":
            return 1.0 / float(self.C)
        return 0.0

    def _objective_from_R(self, R):
        d = self.n_nodes
        if R.shape != (d, d):
            raise ValueError(f"R must have shape ({d}, {d}), got {R.shape}")
        mean_intensity = self.mean_intensity
        covariance = self.covariance
        skewness = self.skewness
        cs_ratio = self.approximate_optimal_cs_ratio() if self.cs_ratio is None else float(self.cs_ratio)

        predicted_covariance = R @ np.diag(mean_intensity) @ R.T
        predicted_skewness = (
            covariance @ np.square(R).T
            + 2.0 * R @ (R * covariance).T
            - 2.0 * R @ np.diag(mean_intensity) @ np.square(R).T
        )
        covariance_divergence = np.mean(np.square(predicted_covariance - covariance))
        skewness_divergence = np.mean(np.square(predicted_skewness - skewness))
        cost = (1.0 - cs_ratio) * skewness_divergence + cs_ratio * covariance_divergence

        if self.strength_lasso > 0.0 or self.strength_ridge > 0.0:
            try:
                adjacency = np.eye(d) - scipy.linalg.inv(R)
            except scipy.linalg.LinAlgError:
                return float("inf")
            if self.strength_lasso > 0.0:
                cost += self.strength_lasso * float(np.sum(np.abs(adjacency)))
            if self.strength_ridge > 0.0:
                cost += self.strength_ridge * float(np.sum(np.square(adjacency)))
        return float(cost)
...
class HawkesCumulantMatchingPyT(HawkesCumulantMatching):
    """PyTorch-backed cumulant-matching optimizer."""
    ...
    def torch_solver(self):
        ...
        if self.solver in {"gd", "momentum"}:
            return torch.optim.SGD
        raise ValueError(f"unsupported PyTorch solver: {self.solver}")
    ...
    def solve(self, adjacency_start=None, R_start=None):
        ...
        torch = self.torch
        start_point = np.real_if_close(
            self._resolve_starting_R(adjacency_start=adjacency_start, R_start=R_start)
        ).astype(float, copy=False)
        previous = self._objective_from_R(start_point)

        R = torch.tensor(start_point, dtype=torch.float64, requires_grad=True)
        mean_intensity = torch.tensor(self.mean_intensity, dtype=torch.float64)
        covariance = torch.tensor(self.covariance, dtype=torch.float64)
        skewness = torch.tensor(self.skewness, dtype=torch.float64)
        cs_ratio = (
            self.approximate_optimal_cs_ratio()
            if self.cs_ratio is None
            else float(self.cs_ratio)
        )
        optimizer_kwargs = dict(self.solver_kwargs)
        optimizer_kwargs.setdefault("lr", float(self.step))
        if self.solver == "momentum":
            optimizer_kwargs.setdefault("momentum", 0.9)
        optimizer = self.torch_solver([R], **optimizer_kwargs)

        last_objective = float(previous)
        rel_obj = math.inf
        n_iter_done = 0
        converged = False
        for n_iter in range(int(self.max_iter)):
            optimizer.zero_grad()
            objective = self._torch_objective_from_R(
                R,
                mean_intensity,
                covariance,
                skewness,
            ...
            if not bool(torch.isfinite(objective).detach().cpu()):
                break
            before_step = float(objective.detach().cpu())
            objective.backward()
            optimizer.step()
            with torch.no_grad():
                current_tensor = self._torch_objective_from_R(
                    R,
                    mean_intensity,
                    covariance,
                    skewness,
            ...
            if not bool(torch.isfinite(current_tensor).detach().cpu()):
                break
            current = float(current_tensor.detach().cpu())
            rel_obj = abs(current - before_step) / max(abs(before_step), 1e-15)
            last_objective = current
            n_iter_done = n_iter + 1
            converged = rel_obj < float(self.tol)
            force = converged or n_iter + 1 == int(self.max_iter)
            if self._should_record_iter(n_iter) or force:
                self._record(
                    n_iter=n_iter + 1,
                    objective=current,
                    rel_obj=rel_obj,
        ...
        solution = R.detach().cpu().numpy()
        if not self._is_valid_solution(solution):
            solution = start_point if self._is_valid_solution(start_point) else np.eye(self.n_nodes)
            last_objective = self._objective_from_R(solution)
        self.solution = solution
        if len(self.history) == 0:
            self._record(
                n_iter=n_iter_done,
                objective=float(last_objective),
                rel_obj=float(rel_obj),
                force=True,
            )
        self.optim_result = {
            "backend": "torch",
    ...
    def _torch_objective_from_R(self, R, mean_intensity, covariance, skewness, cs_ratio):
        torch = self.torch
        predicted_covariance = R @ torch.diag(mean_intensity) @ R.T
        predicted_skewness = (
            covariance @ torch.square(R).T
            + 2.0 * R @ (R * covariance).T
            - 2.0 * R @ torch.diag(mean_intensity) @ torch.square(R).T
        )
        covariance_divergence = torch.mean(torch.square(predicted_covariance - covariance))
        skewness_divergence = torch.mean(torch.square(predicted_skewness - skewness))
        cost = (1.0 - float(cs_ratio)) * skewness_divergence + float(cs_ratio) * covariance_divergence
        if self.strength_lasso > 0.0 or self.strength_ridge > 0.0:
            identity = torch.eye(R.shape[0], dtype=R.dtype, device=R.device)
            adjacency = identity - torch.linalg.inv(R)
            if self.strength_lasso > 0.0:
                cost = cost + float(self.strength_lasso) * torch.sum(torch.abs(adjacency))
            if self.strength_ridge > 0.0:
                cost = cost + float(self.strength_ridge) * torch.sum(torch.square(adjacency))
        return cost


class HawkesCumulantMatchingTf(HawkesCumulantMatchingPyT):
    """Legacy cumulant backend name backed by the PyTorch optimizer."""
    ...
    def __init__(self, *args, **kwargs):
        try:
            ...
            raise ImportError(
                "HawkesCumulantMatchingTf is a legacy name backed by PyTorch; "
                "install the torch extra to use it"
            ) from exc
