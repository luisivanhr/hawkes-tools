...
from hawkes_tools.hawkes.inference import (  # noqa: E402
        ...
        for penalty in [None, "l1", "l2"]:
            with self.subTest(penalty=penalty):
                learner = HawkesCumulantMatchingPyT(
                    ...
                    solver="adam",
                    record_every=1,
                ).fit(events, R_start=np.eye(n_nodes))
                self.assertEqual(learner.optim_result["backend"], "torch")
                self.assertEqual(learner.solution.shape, (n_nodes, n_nodes))
                self.assertEqual(learner.get_kernel_norms().shape, (n_nodes, n_nodes))
                self.assertTrue(np.all(np.isfinite(learner.solution)))
                self.assertTrue(np.isfinite(learner.objective()))

    @unittest.skipIf(importlib.util.find_spec("torch") is None, "PyTorch not installed")
    def test_legacy_tensorflow_name_delegates_to_pytorch_backend(self):
        fixture = _load_train_data()[2.0]
        events = fixture["timestamps"]
        n_nodes = len(events[0])
