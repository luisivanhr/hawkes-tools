...
def classify_tick_test(test_id):
    ...
    if "hawkes_cumulant_matching_test.py" == filename and "_tf_" in method:
        return "pass", "legacy TensorFlow solve variants are covered by the PyTorch production backend"

    if "hawkes_cumulant_matching_test.py" == filename and "_pyt_" in method:
        return "pass", "PyTorch cumulant solve variants are covered by source-backed tests"
...
class TickEquivalenceLedgerTest(unittest.TestCase):
    ...
    def test_ledger_has_both_green_and_gap_work(self):
        ...
        self.assertEqual(counts["skip_optional_backend"], 0)
