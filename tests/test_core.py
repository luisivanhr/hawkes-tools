...
class ModelAndLearnerTest(unittest.TestCase):
    ...
    def test_cumulant_matching_and_validation(self):
        ...
        with self.assertRaisesRegex(ValueError, "solver must be one of"):
            HawkesCumulantMatching(1.0, solver="wrong_name")
        invalid_cases = [
            ((0.0,), {}, "integration_support"),
            ((1.0,), {"C": 0.0}, "C"),
            ((1.0,), {"penalty": "tv"}, "penalty must be one of"),
            ((1.0,), {"step": 0.0}, "step"),
            ((1.0,), {"tol": -1.0}, "tol"),
            ((1.0,), {"max_iter": -1}, "max_iter"),
            ((1.0,), {"print_every": 0}, "print_every"),
            ((1.0,), {"record_every": 0}, "record_every"),
            ((1.0,), {"solver_kwargs": []}, "solver_kwargs"),
            ((1.0,), {"cs_ratio": 1.1}, "cs_ratio"),
            ((1.0,), {"elastic_net_ratio": -0.1}, "elastic_net_ratio"),
        ]
        for args, kwargs, pattern in invalid_cases:
            with self.subTest(args=args, kwargs=kwargs):
                with self.assertRaisesRegex(ValueError, pattern):
                    HawkesCumulantMatching(*args, **kwargs)

    @unittest.skipIf(importlib.util.find_spec("torch") is None, "PyTorch not installed")
    def test_pytorch_cumulant_matching_uses_torch_backend(self):
        start_R = np.eye(2)
        baseline = HawkesCumulantMatching(1.0)
        baseline._set_data(self.events, self.end_time)
        baseline.compute_cumulants(force=True)
        start_objective = baseline.objective(R=start_R)
        learner = HawkesCumulantMatchingPyT(
            ...
            solver="adam",
            record_every=1,
        ).fit(self.events, self.end_time, R_start=start_R)
        self.assertEqual(learner.optim_result["backend"], "torch")
        self.assertEqual(learner.solution.shape, (2, 2))
        self.assertTrue(np.all(np.isfinite(learner.solution)))
        self.assertTrue(math.isfinite(learner.objective()))
        self.assertLessEqual(learner.objective(), start_objective)
        self.assertGreaterEqual(len(learner.history), 1)

    @unittest.skipIf(importlib.util.find_spec("torch") is None, "PyTorch not installed")
    def test_legacy_tf_cumulant_name_uses_torch_backend(self):
        learner = HawkesCumulantMatchingTf(
            ...
            solver="adam",
            record_every=1,
        ).fit(self.events, self.end_time, R_start=np.eye(2))
        self.assertIsInstance(learner, HawkesCumulantMatchingPyT)
        self.assertEqual(learner.optim_result["backend"], "torch")
        self.assertEqual(learner.solution.shape, (2, 2))
        self.assertTrue(math.isfinite(learner.objective()))
