...
from hawkes_tools import plot as hawkes_plot
...
class TickModelPublicBehaviorTest(unittest.TestCase):
    ...
    def test_sumexp_leastsq_varying_baseline_matches_direct_quadrature(self):
        events = [np.array([0.2, 0.9, 1.4]), np.array([0.5, 1.1])]
        end_time = 2.0
        decays = np.array([0.7, 1.3])
        period_length = 1.5
        n_baselines = 3
        model = ModelHawkesSumExpKernLeastSq(
            decays, n_baselines=n_baselines, period_length=period_length
        ).fit(events, end_time)
        coeffs = np.array(
            [
                0.25,
        ...
        baseline, adjacency = model._unpack(coeffs)
        points = sorted(
            {
                *[float(t) for node_events in events for t in node_events],
                period_length / n_baselines,
                2.0 * period_length / n_baselines,
                period_length,
            }
        )
        expected = 0.0
        for target in range(2):
            integral = quad(
                lambda t, target=target: _sumexp_intensity_from_coeffs(
                    events, baseline, adjacency, decays, target, float(t), period_length
                )
                ** 2,
                0.0,
                end_time,
                points=[point for point in points if 0.0 < point < end_time],
                epsabs=1e-10,
                epsrel=1e-10,
            )[0]
            event_sum = sum(
                ...
                for t in events[target]
            )
            expected += integral - 2.0 * event_sum

        self.assertAlmostEqual(model.loss(coeffs) * model.n_jumps, expected, places=8)
        grad = model.grad(coeffs)
        eps = 1e-6
        for index in [0, 3, 6, 9, 13]:
            step = np.zeros_like(coeffs)
            step[index] = eps
            numeric = (model.loss(coeffs + step) - model.loss(coeffs - step)) / (2.0 * eps)
...
class TickDocumentedGapCompatibilityTest(unittest.TestCase):
    ...
    def test_deep_tick_hawkes_import_paths(self):
        cases = [
            ...
            ("hawkes_tools.hawkes.simulation.hawkes_kernels", "HawkesKernelExp"),
            (
                "hawkes_tools.hawkes.simulation.hawkes_kernels.hawkes_kernel_exp",
                "HawkesKernelExp",
            ),
            ("hawkes_tools.hawkes.simulation.simu_hawkes", "SimuHawkes"),
            ("hawkes_tools.hawkes.model", "ModelHawkesExpKernLeastSq"),
            ("hawkes_tools.hawkes.model.base", "ModelHawkes"),
            (
                "hawkes_tools.hawkes.model.model_hawkes_sumexpkern_leastsq",
                "ModelHawkesSumExpKernLeastSq",
            ),
            ("hawkes_tools.hawkes.inference.base", "LearnerHawkesNoParam"),
            ("hawkes_tools.hawkes.inference.hawkes_em", "HawkesEM"),
            (
                "hawkes_tools.hawkes.inference.hawkes_cumulant_matching",
                "HawkesTheoreticalCumulant",
            ),
        ]
        for module_name, attr in cases:
