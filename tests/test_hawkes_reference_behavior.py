...
class ConditionalLawReferenceBehaviorTest(unittest.TestCase):
    ...
    def test_conditional_law_log_kernel_values_preserve_invalid_signed_estimates(self):
        learner = HawkesConditionalLaw(quad_method="log")
        learner.kernels = [[np.array([[0.0, 1.0, 10.0, 100.0], [1.0, 0.1, -0.01, 0.001]])]]

        x = np.array([1.0, 5.0, 10.0, 50.0, 1000.0])
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            expected = np.power(
                ...
                np.interp(
                    np.log10(x),
                    np.log10(learner.kernels[0][0][0]),
                    np.log10(learner.kernels[0][0][1]),
                    left=np.nan,
                    right=np.nan,
                ),
            )
