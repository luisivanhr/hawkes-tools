...
class SimulationParityTest(unittest.TestCase):
    ...
    def test_exp_kernel_fresh_simulation_is_seeded_and_sorted(self):
        adjacency = np.array(
            [
                [0.25, 0.25, -0.125],
                [0.0, 0.25, 0.0],
        ...
        def simulate():
            ...
            simu.threshold_negative_intensity()
            return simu.simulate()

        first = simulate()
        second = simulate()

        self.assertAlmostEqual(first.simulation_time, 1_000.0)
        self.assertGreater(first.n_total_jumps, 500)
        self.assertEqual(first.n_total_jumps, second.n_total_jumps)
        assert_timestamps_equal(self, first.timestamps, second.timestamps)
        for node_timestamps in first.timestamps:
            self.assertTrue(np.all(np.isfinite(node_timestamps)))
            self.assertTrue(np.all(np.diff(node_timestamps) >= 0.0))
            self.assertTrue(np.all(node_timestamps < first.end_time))

    def test_mixed_exp_time_function_fast_path_matches_generic_path(self):
        t_values = np.array([0.0, 0.4, 1.0, 1.5])
        y_values = np.array([0.0, 0.3, 0.1, 0.0])
        time_kernel = HawkesKernelTimeFunc(
            TimeFunction([t_values, y_values], inter_mode=TimeFunction.InterConstRight)
        )

        def build(track=False):
            simu = SimuHawkes(baseline=[0.2, 0.15], end_time=80.0, seed=2334, verbose=False)
            simu.set_kernel(0, 0, time_kernel)
            simu.set_kernel(0, 1, HawkesKernelExp(0.25, 0.7))
            simu.set_kernel(1, 1, time_kernel)
            if track:
                simu.track_intensity(100.0)
            return simu

        fast = build(track=False).simulate()
        generic = build(track=True).simulate()

        self.assertEqual(fast.n_total_jumps, generic.n_total_jumps)
        for actual, expected in zip(fast.timestamps, generic.timestamps):
